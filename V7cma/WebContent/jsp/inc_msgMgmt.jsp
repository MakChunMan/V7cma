...
for (int x= 0; x<sellItemList.size(); x++){
	...
	for(int y =0; y < ((List)enqList.get(x)).size(); y++){
		...
		if(tmpEnq.getParentid() != previd){
			...
			out.println("<!--tr>");
			out.println("<td colspan=\"2\"><img id=\"new"+ tmpEnq.getParentid()+ "\" src=\""+ staticPath + "/images/unread.png\" style=\"display:none\" alt=\""+ MessageUtil.getV6Message(lang,"ENQ_NEW") + "\">" + 
					(CommonUtil.isNullOrEmpty(tmpItem.getMessageContent())?
							MessageUtil.getV6Message(lang,"TXT_ORDER_NO")+ ": "+ tmpItem.getContentid():
							tmpItem.getMessageContent())	
					+ "</td></tr-->\n");
			out.println("<tr><td width=\"35%\">"+  
