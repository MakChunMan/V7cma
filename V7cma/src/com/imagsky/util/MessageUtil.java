...
import com.imagsky.v6.domain.StringMessage;
import com.imagsky.v6.domain.StringMessagePK;
...
public class MessageUtil {
		...
		if(param==null){
			...
			for (int x= 0 ; x< param.size(); x++){
				try{
					...
					cmaLogger.error("replaceAll Exception: " + paramSymbol + (x+1) + paramSymbol, e);
				}
			}
			return str;
		}
	...
	public static String getMessage(String mod, String lang, String appCode, boolean returnCodeIfEmpty){
		...
		if(str==null || CommonUtil.isNullOrEmpty(str.getStr_value())){
			cmaLogger.info("[STR_MISSING] "+appCode);
	...
	public static String getV6Message(String lang, String appCode){
		return getMessage(MOD_V6, lang, appCode);
	}
	
	public static String getV6Message(String lang, String appCode, ArrayList<String> param){
