...
public abstract class AbstractDbDAO {
    ...
    public List<Object> CNT_findListWithSample(Object enqObj, Integer startRow, Integer chunksize, String orderByField) throws BaseDBException {
        ...
        try {
            ...
            cmaLogger.error("IllegalAccessException:", ex);
        } catch (IllegalArgumentException ex) {
            cmaLogger.error("IllegalArgumentException:", ex);
        } catch (InvocationTargetException ex) {
            cmaLogger.error("InvocationTargetException:", ex);
        } catch (NoSuchMethodException ex) {
            cmaLogger.error("NoSuchMethodException:", ex);
        } catch (SecurityException ex) {
            cmaLogger.error("SecurityException:", ex);
        } catch (Exception ex) {
            cmaLogger.error("Exception:", ex);
        }
        return null;
    }
    ...
    protected void setLogger(cmaLogger LOGGER) {
        this.LOGGER = LOGGER;
    }
    ...
    protected void throwException(String message, String sql, Throwable t) throws BaseDBException {
        if (t instanceof GenericDatabaseException) {
            GenericDatabaseException databaseException = (GenericDatabaseException) t;
            throw new BaseDBException(message, "ErrorCodeConstants.ERR_SQL_ERROR_ABSTRACT_DBDAO", sql, t);

            //throw new DbDAOException(message, sql, t);
/*
             * Throwable target = databaseException.getRootException(); if
             * (target == null) { throw new DbDAOException(message, sql, t); }
             * else { throw new DbDAOException(message, sql, target); }
             */
        } else {
    ...
    //protected abstract void beanValidate(Object entityObj) throws BaseDBException;
