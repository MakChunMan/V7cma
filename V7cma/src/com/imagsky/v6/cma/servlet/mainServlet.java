...
import com.imagsky.servlet.handler.V7HandlerFactory;
...
/**
 * Servlet implementation class mainServlet 2013003-05 V7 implementation - /cma
 * for Content Management portal
 */
public class mainServlet extends HttpServlet {
    ...
    public void init() throws ServletException {
        ...
        //Clear file public folder
        ClearFileUtil.clearPublicFile();
    }
    ...
    protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
        ...
        requestURI = request.getRequestURI(); //cmaLogger.debug("URI" + requestURI);
        request.setAttribute("requestURI", requestURI);
        contextPath = request.getContextPath(); //cmaLogger.debug("contextPAth" + contextPath);
        ...
        cmaLogger.info("[REQUEST_INFO] " + request.getRemoteAddr() + "|" + fullRequestURL + "|" + CommonUtil.null2Empty(request.getParameter("v")) + "|" + referer, request);
        ...
        try {
            ...
            try {
                ...
                cmaLogger.debug("Init appCode:" + appCode);
                appCode = appCodeToken[0];
                request.setAttribute(SystemConstants.REQ_ATTR_URL_PATTERN, appCodeToken);
                cmaLogger.debug("REQ_ATTR_URL_PATTERN size:" + ((String[]) request.getAttribute(SystemConstants.REQ_ATTR_URL_PATTERN)).length);
            } catch (Exception e) {
                cmaLogger.warn("[SUSPECT_URL] AppCode = " + appCode, request);
            }
            ...
            *	 *******************************************
            ...
            if (V6Util.RedirectToLogin(request, appCode, isV7)) { //Compatible wihth V7
                ...
                cmaLogger.info("[REDIRECT] LOGIN|" + actionResponse.getTargetJSP(), request);
                throw new Exception();
            }
            if ("Y".equalsIgnoreCase((String) request.getAttribute("SYS_SECURE"))
                ...
                cmaLogger.info("[REDIRECT] SECURE|" + actionResponse.getTargetJSP(), request);
                throw new Exception();
            }
            /**
             * *****************************************
		 ...
		 ******************************************
             */
            cmaLogger.debug("Entering MainServlet : doRequest [ STEP 3. session id = " + session.getSessionId() + " appcode = " + appCode + "; action = " + action, request);
            cmaLogger.info("[APP] " + (isV7 ? "V7|" : "") + appCode + "|" + action, request);
            ...
            if (isValidURL && !appCode.equals("")) {
                ...
                try {
                    ...
                    cmaLogger.error("[ STEP 3. ActionHandler exception] ", request, e);
                } // execute action
                long actionTimeTaken = (System.currentTimeMillis() - actionStart);

                cmaLogger.debug("Entering MainServlet : doRequest [ STEP 3. ActionHandler TimeTaken = " + actionTimeTaken + " ms", request);
                cmaLogger.info("[APP_PFM] " + appCode + "|" + action + "|" + actionTimeTaken + "ms", request);
                ...
                if (actionResponse.getTargetJSP() != null && !"".equals(actionResponse.getTargetJSP())) {
                    ...
                    cmaLogger.debug("Entering MainServlet : doRequest [ STEP 3.3. Override forwardURL = " + forwardURL, request);
                }
            }
		 ...
		 ******************************************
             */
            if (true) {
                cmaLogger.debug("Entering MainServlet : doRequest [ STEP 4. forward to URL-- Continue]", request);
                // 11. forward to URL
                // ------------------
                cmaLogger.debug("URL forwarding " + forwardURL, request);
                ...
                cmaLogger.debug("Entering MainServlet : doRequest [ STEP 4. forward to URL-- handle error URL]", request);
                // 11. handle error URL
                // --------------------
...
//			dispatchUndefinedErrorPage(request, response);
            ...
            cmaLogger.debug("[Exception mainServlet debug] actionResponse "+actionResponse);
            cmaLogger.debug("[Exception mainServlet debug] action "+action);
            if (actionResponse != null && CommonUtil.null2Empty(action).indexOf("AJ") == -1) {
                ...
                cmaLogger.debug("Entering MainServlet : doRequest [ END. Redirect to Login Page]", request);
            } else {
                dispatcher = getServletContext().getRequestDispatcher("/error.jsp");
                cmaLogger.error("Unknown exception in mainServlet: ", request, e);
            }
            dispatcher.forward(request, response);
        }
    }
